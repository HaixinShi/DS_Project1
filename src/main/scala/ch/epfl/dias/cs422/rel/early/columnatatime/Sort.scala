...
class Sort protected (
  ...
  override def execute(): IndexedSeq[HomogeneousColumn] = {
    ...
    val tuples = (cols.transpose filter { tuple:Tuple => tuple(cols.size - 1) match { case b: Boolean => b } }).map(row => (0 until(row.size - 1)).map(i => row(i)))
    ...
    results.transpose.map(i => toHomogeneousColumn(i)) :+ toHomogeneousColumn((0 until results.transpose.head.size).toArray.map(_ => true))//(0 until results.transpose.head.size).map(_ => true)
  }
}
